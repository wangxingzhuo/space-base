const dict = [
  ...
  pre.set(item[0], item[1]);
  return pre;
}, new Map<string, string>());

export function mimeDict(extName: string): string {
  return dict.has(extName) ? dict.get(extName) : '';
};

export function int64(a: i64): i32[] {
  return [i32((a >> 32) & 0xFFFFFFFF), i32(a & 0xFFFFFFFF)];
}

export function toInt64(a: i32, b: i32): i64 {
  return (a << 32) | b;
}
...
export function findIndex(ar: IFoo[], x: i32): i32 {
  let e: i32 = ar.length - 1;
  // lt ar
  if (x < ar[0].start) return -1;
  const _start = ar[e].start;
  const _duration = ar[e].duration;
  // gt ar
  ...
  do {
    let i = (s + e) >> 1;
    const start = ar[i].start;
    const duration = ar[i].duration;
...
export function limitArea(x: i32, min: i32 = -2147483648, max: i32 = 2147483647): i32 {
  if (x < min) return min;
  if (max < x) return max;
  return x;
}
...
class Media implements IMedia {
  ...
  constructor(s: f64, d: f64, t: string, u: string) {
    this.duration = d;
    this.start = s;
    this.url = u;
    this.title = t;
...
export function parseM3U(m3u: string): IMedia[] {
